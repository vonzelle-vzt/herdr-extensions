...
| `ctrl+b` `shift+z` | **Spaces** — list every herdr workspace; focus, rename, or close one. |
...
| **Spaces** | Lists every herdr workspace — label, pane/tab counts, agent status, which is focused — with a small command line: a number focuses it, `r <number>` renames it (and tells you renaming after a project directory is what makes the editor auto-open there next time), `x <number>` closes it after you type back `CLOSE <label>` to confirm. This exists because right-click is the discoverable way to rename or close a space and it is **unreliable on Terminal.app**, which swallows Button3 — the only paths left are keybindings nobody remembers. **Refuses outright to close the space it is running in** — that would end the panel mid-action with nobody left to see the result. |
